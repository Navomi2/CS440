...
class MazeState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        nbr_states = []
        neighboring_locs = self.maze_neighbors(self.state[0],self.state[1],self.state[2])
        ...
        for neighbor_loc in neighboring_locs:
            ...
            cost = euclidean_distance(self.state, neighbor_loc)
            if (neighbor_loc[2] != self.state[2]): #shape changes
                cost = 10

            new_state = MazeState(
                    state = (neighbor_loc[0], neighbor_loc[1], neighbor_loc[2]),
                    goal= new_goal,
                    dist_from_start=self.dist_from_start + cost,
                    maze=self.maze,
    ...
    # TODO VI
    def compute_heuristic(self):
        nearest_goal_dist = euclidean_distance((self.state[0], self.state[1]), self.goal[0])
        for goal in self.goal:
            new_dist = euclidean_distance((self.state[0], self.state[1]), goal)
            if new_dist < nearest_goal_dist:
                nearest_goal_dist = new_dist
